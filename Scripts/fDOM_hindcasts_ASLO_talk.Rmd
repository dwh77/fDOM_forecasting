...
#join climatology to AR and NNETAR
AR_clim_nnetar_outputs <- full_join(ARoutputs, climatology_outputs, by = c("datetime", "reference_datetime", "site_id")) |>
...
#join obersved data
fcr_bvr_fdom_observed <- targets_df |> 
  filter(variable == "fDOM_QSU_mean") |> 
  select(site_id, datetime, observation)
 
AR_clim_nnetar_outputs_observed <- left_join(AR_clim_nnetar_outputs, fcr_bvr_fdom_observed, by = c("datetime", "site_id")) |> 
...
#rmse
rmsefig <- AR_clim_nnetar_outputs_observed |> 
...
#sd
sdfig <- AR_clim_nnetar_outputs_observed |> 
  group_by(Horizon, site_id) |> 
...
#fdom_observed_ts <- 
AR_clim_nnetar_outputs_observed |> 
