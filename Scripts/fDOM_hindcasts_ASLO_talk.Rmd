...
#rmse
rmsefig_allyear_AR <- AR_clim_nnetar_persist_outputs_observed |> 
  ...
  filter(model == "Autoregressive",
         Horizon > 0,
         Horizon < 17) |>
  mutate(site_id = ifelse(site_id == "bvre", "Beaverdam Reservoir (BVR)", "Falling Creek Reservoir (FCR)")) |> 
...
ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/rmse_ar_allyear_V2.jpg",
       rmsefig_allyear_AR, device = "jpg", width = 250, height = 150, units = "mm")


rmsefig_allyear_allmodel <- AR_clim_nnetar_persist_outputs_observed |> 
  filter(reference_datetime >= ymd("2023-02-01")) |> 
  group_by(Horizon, site_id) |> 
  summarise(Autoregressive = round(sqrt(mean((mu_AR - observation)^2, na.rm = TRUE)), 2),
            Climatology = round(sqrt(mean((mu_clim - observation)^2, na.rm = TRUE)), 2),
...
ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/rmse_allmodel_allyear_V2.jpg",
       rmsefig_allyear_allmodel, device = "jpg", width = 250, height = 150, units = "mm")

#sd
sd_fig <- AR_clim_nnetar_persist_outputs_observed  |>
  filter(reference_datetime >= ymd("2023-02-01")) |>
  group_by(Horizon, site_id) |>
  summarise(Autoregressive = mean(sigma_AR, na.rm = T),
            Climatology = mean(sigma_clim, na.rm = T),
            NNETAR = mean(sigma_NNETAR, na.rm = T),
            Persistence = mean(sigma_persist, na.rm = T)) |>
  pivot_longer(-c(1:2)) |>
    rename(model = name) |> 
  filter(Horizon > 0,
         Horizon < 17) |>
  ...
  scale_x_continuous(breaks = c(1,3,5,7,9,11,13,15))+
  #ggtitle("SD")+
  labs(x = "Horizon", y = "SD (QSU)")+
  theme_bw() + theme(legend.position = "top", text = element_text(size = 18))+
    scale_color_manual(values = c("Autoregressive" = "#CC79A7", "Climatology" = "#E69F00", 
                                "Persistence" = "#56B4E9", "NNETAR" = "#009E73"))

ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/SD_allmodels.jpg",
       sd_fig, device = "jpg", width = 250, height = 150, units = "mm")
...
fdom_dec22_jan24 <- AR_clim_nnetar_persist_outputs_observed |> 
  mutate(site_id = ifelse(site_id == "bvre", "Beaverdam Reservoir (BVR)", "Falling Creek Reservoir (FCR)")) |> 
...
ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/fdom_obs_dec22_jan24.jpg",
       fdom_dec22_jan24, device = "jpg", width = 250, height = 150, units = "mm")


fdom_alltime <- fcr_bvr_fdom_observed |> 
  mutate(site_id = ifelse(site_id == "bvre", "Beaverdam Reservoir (BVR)", "Falling Creek Reservoir (FCR)")) |> 
  ggplot(aes(x = datetime, y = observation, color = site_id))+
  geom_point()+
  #ggtitle("FCR and BVR")+
...
ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/fdom_obs_alltime.jpg",
       fdom_alltime, device = "jpg", width = 250, height = 150, units = "mm")
...
rmse_season_full <- AR_clim_nnetar_persist_outputs_observed |> 
  filter(reference_datetime >= ymd("2023-02-01")) |> 
  filter(Horizon > 0,
         Horizon < 17) |> 
  ...
  rename(model = name) |> 
  mutate(site_id = ifelse(site_id == "bvre", "Beaverdam Reservoir (BVR)", "Falling Creek Reservoir (FCR)")) |> 
  ggplot(aes(x = Horizon, y = value, color = model, shape = Season, linetype = Season))+ # 
  geom_line(linewidth = 1.2)+
  geom_point(size = 3)+
  ...
  labs(x = "Horizon", y = "RMSE (QSU)")+
    ylim(0,3.2)+
  theme_bw() + theme(legend.position = "top", text = element_text(size = 18))+
  scale_color_manual(values = c("Autoregressive" = "#CC79A7", "Persistence" = "#56B4E9"))

#Default ggpot colors "Autoregressive" = "#F8766D", "Persistence"= "#F564E3" NNETAR = #00BFC4 , Clim = #00BA38


ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/rmse_sseasons_ar_persist_v2.jpg",
       rmse_season_full, device = "jpg", width = 250, height = 150, units = "mm")


rmse_season_summer <- AR_clim_nnetar_persist_outputs_observed |> 
  filter(reference_datetime >= ymd("2023-02-01")) |> 
  filter(Horizon > 0,
         Horizon < 17) |> 
  mutate(Month = month(as.Date(datetime))) |> 
...
ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/rmse_ssummer_ar_persist_V2.jpg",
       rmse_season_summer, device = "jpg", width = 250, height = 150, units = "mm")
...
##get week before and example forecast
weekbefore <- fcr_bvr_fdom_observed |> 
    filter(datetime <= ymd("2023-07-15"),
           datetime >= ymd("2023-07-15") - 7) 

example_forecast_data <- combined_for_plots |> 
  filter(reference_datetime == ymd("2023-07-15"))
  
### just observations
pre_forecast_fig <- example_forecast_data |> 
  filter(model_id == "AR") |> 
  rbind(weekbefore) |> 
  mutate(observation = ifelse(datetime > ymd("2023-07-15"), NA, observation)) |> 
  mutate(site_id = ifelse(site_id == "bvre", "Beaverdam Reservoir (BVR)", "Falling Creek Reservoir (FCR)")) |> 
  ggplot()+
  geom_point(aes(datetime, observation, fill = model_id)) +
...
ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/pre_4cast_fig_V2.jpg",
       pre_forecast_fig, device = "jpg", width = 250, height = 150, units = "mm")

### pre observations and AR forecast

ar_forecast_fig <- example_forecast_data |> 
  filter(model_id == "AR") |> 
  rbind(weekbefore) |> 
  mutate(observation = ifelse(datetime > ymd("2023-07-15"), NA, observation)) |> 
    mutate(model_id_full = ifelse(model_id == "AR", "Autoregressive", model_id_full)) |> 
    mutate(site_id = ifelse(site_id == "bvre", "Beaverdam Reservoir (BVR)", "Falling Creek Reservoir (FCR)")) |> 
  ggplot()+
  geom_point(aes(datetime, observation)) +
  geom_vline(aes(xintercept = as.POSIXct("2023-07-15")))+
  geom_ribbon(aes(x = datetime, ymin = quantile02.5, ymax = quantile97.5, fill = model_id_full), alpha = 0.2)+
  geom_line(aes(x = datetime, y = mean, col = model_id_full))+
  facet_wrap(~site_id, scales = "fixed") +
  labs(x = 'datetime', y = 'fDOM (QSU)', fill = "model", color = "model") +
  ylim(0,40)+
  guides(x =  guide_axis(angle = 30)) +
  theme_bw() + theme(legend.position = "top", text = element_text(size = 18))+
  scale_color_manual(values = c("Autoregressive" = "#CC79A7"))+
  scale_fill_manual(values = c("Autoregressive" = "#CC79A7"))

ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/ar_4cast_fig_V2.jpg",
       ar_forecast_fig, device = "jpg", width = 250, height = 150, units = "mm")

## pre observations and all forecasts
all_forecast_fig <- example_forecast_data |> 
  rbind(weekbefore) |> 
  mutate(observation = ifelse(datetime > ymd("2023-07-15"), NA, observation)) |> 
  mutate(model_id_full = ifelse(model_id == "clim", "Climatology", NA),
         model_id_full = ifelse(model_id == "AR", "Autoregressive", model_id_full),
         model_id_full = ifelse(model_id == "persist", "Persistence", model_id_full),
         model_id_full = ifelse(model_id == "ML", "NNETAR", model_id_full)) |> 
    mutate(site_id = ifelse(site_id == "bvre", "Beaverdam Reservoir (BVR)", "Falling Creek Reservoir (FCR)")) |> 
  ggplot()+
  geom_point(aes(datetime, observation)) +
  geom_vline(aes(xintercept = as.POSIXct("2023-07-15")))+
  geom_ribbon(aes(x = datetime, ymin = quantile02.5, ymax = quantile97.5, fill = model_id_full), alpha = 0.2)+
  geom_line(aes(x = datetime, y = mean, color = model_id_full))+
  facet_wrap(~site_id, scales = "fixed") +
  labs(x = 'datetime', y = 'fDOM (QSU)', fill = "model", color = "model") +
  ylim(0,40)+
  guides(x =  guide_axis(angle = 30)) +
  ...
  scale_color_manual(values = c("Autoregressive" = "#CC79A7", "Climatology" = "#E69F00", 
                                "Persistence" = "#56B4E9", "NNETAR" = "#009E73"))+
  scale_fill_manual(values = c("Autoregressive" = "#CC79A7", "Climatology" = "#E69F00", 
                                "Persistence" = "#56B4E9", "NNETAR" = "#009E73"))



ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/all_4cast_fig_V2.jpg",
       all_forecast_fig, device = "jpg", width = 250, height = 150, units = "mm")


## all observations and  forecasts
all_forecast_fig_withobs <- example_forecast_data |> 
  rbind(weekbefore) |> 
  mutate(model_id_full = ifelse(model_id == "clim", "Climatology", NA),
         model_id_full = ifelse(model_id == "AR", "Autoregressive", model_id_full),
         model_id_full = ifelse(model_id == "persist", "Persistence", model_id_full),
...
ggsave(filename = "C:/Users/dwh18/OneDrive/Desktop/R_Projects/fDOM_forecasting/Data/ASLO_talk_forecast_output/all_4cast_fig_withobs_V2.jpg",
       all_forecast_fig_withobs, device = "jpg", width = 250, height = 150, units = "mm")
