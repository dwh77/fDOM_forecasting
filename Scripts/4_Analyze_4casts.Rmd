...
Scored_forecasts |> 
  ...
  ggplot(aes(x = Horizon, y = CRPS, color = model_id))+ # color = model_id
  geom_line(linewidth = 1.2)+
  geom_point(size = 3)+
  facet_wrap(~site_id)+
  #facet_grid(model_id~site_id)+
  scale_x_continuous(breaks = c(1,3,5,7,9,11,13,15))+
  labs(x = "Horizon", y = "CRPS (QSU)")+
  theme_bw() + theme(legend.position = "top", text = element_text(size = 18))+
  scale_color_manual(values = c("AR" = "#CC6677", "Clim" = "#88CCEE", "Persist" = "#DDCC77", 
                              "NNETAR" = "#117733", "RainLag" = "#332288", "MetRegress" = "#888888"))
...
Scored_forecasts |> 
  ...
  ggplot(aes(x = Horizon, y = SD, color = model_id))+ # color = model_id
  geom_line(linewidth = 1.2)+
  geom_point(size = 3)+
  facet_wrap(~site_id)+
  #facet_grid(model_id~site_id)+
...
Scored_forecasts |> 
  ...
  summarise(RMSE = round(sqrt(mean((mean - observation)^2, na.rm = TRUE)), 2),
            ...
            SD = mean(sd, na.rm = T)) |>
  pivot_longer(-c(1:3)) |> 
  ggplot(aes(x = Horizon, y = value, color = model_id))+ # color = model_id
  geom_line(linewidth = 1.2)+
  geom_point(size = 3)+
  facet_grid(name~site_id, scales = "free_y")+
  scale_x_continuous(breaks = c(1,3,5,7,9,11,13,15))+
